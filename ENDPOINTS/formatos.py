...
async def exportar_f165(request: ExportarF165Request, db: Session = Depends(get_db)):
    ...
    if modalidad == "individual" and aprendices:
        ...
        nombre_interno = f"F165_{usuario_gene.id}_{request.ficha}_{aprendiz_documento}_{timestamp}.xlsx"
    else:
        nombre_original = f"F165_{modalidad}_{request.ficha}_{timestamp}.xlsx"
        nombre_interno = f"F165_{usuario_gene.id}_{request.ficha}_{timestamp}.xlsx"
